...
public class HorizontalScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    public int CurrentScreen()
    {
        ...
        absPoz = Mathf.Clamp(absPoz, 1, _itemSize - 1);

        float calc = (absPoz / _itemSize) * _screens;
    ...
    {
        int _offset = 0;
        int _step = (int)_screensContainer.transform.GetChild(0).gameObject.GetComponent<RectTransform>().rect.width; ;
        int _dimension = 0;
