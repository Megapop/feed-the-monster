...
public class Tutorial : MonoBehaviour
{
        ...
        try
        {
            switch (State)
            {
                case TutorialController.TutorialState.Pointing:
                    if (counter >= 1)
                    { // finished animation
                        if (Vector3.Distance(pointAnimationEnd, pointLocation) < 1)
                        { // finished going to point
    ...
    public void InitGame(Transform holder)
    {
        ...
        StartCoroutine(FeedGameLetter(holder, 2.5f));
    }


    private IEnumerator FeedGameLetter(Transform holder, float delay = 0)
    {
        ...
        for (int l = 0; l < lettersOriginal.Length; l++)
        {
            ...
            tempContainer = (GameObject)Instantiate(curContainer.gameObject, curContainer.position, curContainer.rotation, holder);
        ...
        if (!string.IsNullOrEmpty(GameplayController.Instance.CurrentSegment.VoiceTutorial) && isVoiceShow == false)
        {
            TutorialController.Instance.AddVoiceTutorial(GameplayController.Instance.CurrentSegment.VoiceTutorial);
            isVoiceShow = true;
        }
        ...
        if (letters.Length != 0)
        {
            ...
            yield return FeedGameLetter(holder, delay);
            tutorialHandImage.transform.position = handDest;
