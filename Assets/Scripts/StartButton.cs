...
//		slidingIn = true;
	...
	void startStep(StepColors color)
	{
		...
		Timer.Instance.Remove (OnStepDone);
		Timer.Instance.Create (new Timer.TimerCommand (stepTime, OnStepDone));
//		updateColor ();
	}

	void OnStepDone(float t)
	{
...
*/
	...
	void playAnimation()
	{
		...
		if (GameplayController.Instance.pointTutorial != null) {
			TutorialController.Instance.EndTutorial (GameplayController.Instance.pointTutorial);
		}
