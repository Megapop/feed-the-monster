...
//	}
	...
	public IEnumerator LoadFromResources(int levelId, string levelPath, Level level){
		...
		if (xmlDataFile == null) {
			Debug.Log ("Level file is missing: " + levelPath);
			level = null;
		} else {
	...
	private void SerializeLevelFromXML(int levelId, string levelPath, XmlReader reader, Level level) {
		...
		if(level.monsterInputType == MonsterInputType.Letter || level.monsterInputType == MonsterInputType.LetterName || level.monsterInputType == MonsterInputType.LetterInWord || level.monsterInputType == MonsterInputType.SoundLetter || level.monsterInputType == MonsterInputType.SoundLetterName) {
